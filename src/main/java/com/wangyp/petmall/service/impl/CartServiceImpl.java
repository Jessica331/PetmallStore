...
import com.wangyp.petmall.entity.Cart;
import com.wangyp.petmall.entity.CartVO;
import com.wangyp.petmall.entity.Product;
import com.wangyp.petmall.mapper.CartMapper;
...
public class CartServiceImpl implements ICartService {
     ...
     */
    @Override
    public void delToCart(Integer uid, Integer pid) {
        // 根据参数pid和uid查询购物车中的数据
        Cart result = cartMapper.findByUidAndPid(uid, pid);
        Date now = new Date();
        // 判断查询结果是否为null
        if (result == null) {
            throw new DeleteException("商品商品数据时出现未知错误，请联系系统管理员");
        } else {
            // 否：表示该用户的购物车中已有该商品
            // 从查询结果中获取购物车数据的id
            Integer cid = result.getCid();
            ...
            if (rows != 1) {
                throw new DeleteException("删除商品时出现未知错误，请联系系统管理员");
            }
        }
    }
    ...
    public Integer reduceNum(Integer cid, Integer uid, String username) {
        ...
        if (result.getNum() > 0) {
            ...
            throw new UpdateException("商品数量小于0");
        }
